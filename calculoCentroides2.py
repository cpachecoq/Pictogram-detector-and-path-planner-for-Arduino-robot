...
class IntegratedApp(QWidget):
    def __init__(self):
        ...
        self.cap = cv2.VideoCapture(1)  # Ajusta según tu cámara
        self.model = YOLO("best20.pt")   # Modelo YOLOv8 entrenado
    ...
    def update_frame(self):
        ...
        # Detección con YOLO
        results = self.model.predict(source=rgb, conf=0.3, verbose=False)
        for box in results[0].boxes:
            ...
            if cls_id == 0:  
                ...
                centroid_chakana = (cx, cy)
                if centroid_chakana:
                    cv2.circle(rgb, centroid_chakana, 6, (0, 0, 255), -1)
                    cv2.putText(rgb, f"Chakana ({cx},{cy})",
                                (cx + 10, cy), 
